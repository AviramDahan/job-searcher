...
- Use the public CBS locality database in `data/public/israel_localities.json` so radius search and the map include cities, towns, villages, moshavim, and kibbutzim beyond the hand-picked location list.
...
Refresh the Israel localities database from data.gov.il when CBS publishes a newer file:

```powershell
python -m src.update_israel_localities --out .\data\public\israel_localities.json
python -m src.export_pages_dashboard --csv .\outputs\job_applications.csv --summary .\outputs\job_search_summary.md --out .\docs\assets\job-data.json --candidate-name "קורן דהן"
```

The locality source is the official CBS `localities-in-israel` dataset. The script reads DataStore records, converts Israel TM Grid / EPSG:2039 coordinates to WGS84, and writes only public geography metadata.
